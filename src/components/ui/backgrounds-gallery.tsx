...
export default function BackgroundsGallery({
  ...
  // Derive activeCategory
  const activeCategory =
    activeCategoryOverride ??
    (CATEGORIES.some((c) => c.id === catParam) ? catParam : "all");
  ...
  const filteredBackgrounds = useMemo(() => {
    return initialBackgrounds.filter((bg) => {
      const matchesCategory =
        activeCategory === "all" ||
        bg.category.toLowerCase() === activeCategory.toLowerCase();
      const matchesSearch =
          ...
          <div className="flex sm:flex-wrap gap-2 overflow-x-auto pb-2 sm:pb-0 snap-x snap-mandatory [-ms-overflow-style:none] scrollbar-none">
            {CATEGORIES.map((category) => {
              const count =
                category.id === "all"
                  ? initialBackgrounds.length
                  : initialBackgrounds.filter(
                      (b) => b.category.toLowerCase() === category.id
                    ).length;
              ...
              const href =
                category.id === "all"
                  ? "/backgrounds"
                  : `/backgrounds/${category.id}`;

              return (
                <Link
                  key={category.id}
                  href={href}
                  className={`px-4 py-2 rounded-pill text-xs font-semibold transition-all duration-200 border cursor-pointer flex items-center gap-2 shrink-0 snap-start ${
                    isActive
      ...
      {filteredBackgrounds.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center rounded-card border border-dashed border-border-light bg-surface-2/10">
          ...
          <button
            onClick={() => {
              setSearchQuery("");
              if (pathname !== "/backgrounds") {
                router.push("/backgrounds");
              }
            }}
            className="mt-4 flex items-center gap-1.5 text-xs text-primary hover:underline font-semibold cursor-pointer"
          >
